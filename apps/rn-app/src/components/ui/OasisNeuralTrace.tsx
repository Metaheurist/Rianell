...
/** Bioluminescent neural trace — opacity pulse (native-driver safe). */
export function OasisNeuralTrace({ color = 'rgba(123,223,140,0.5)', height = 80 }: Props) {
  ...
  return (
    <Animated.View style={[styles.wrap, { height, opacity }]} pointerEvents="none" accessibilityElementsHidden>
      <Svg width="100%" height={height} viewBox={`0 0 400 ${height}`} preserveAspectRatio="none">
        <Path d={d} fill="none" stroke={color} strokeWidth={1.5} strokeDasharray="12 8" opacity={0.85} />
      </Svg>
    </Animated.View>
  );
}
