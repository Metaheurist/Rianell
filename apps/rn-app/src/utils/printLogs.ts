...
export async function printOrShareLogs(logs: LogEntry[], locale = 'en-GB'): Promise<void> {
  const rows = logs
    .map(
      (e) =>
        `<tr><td>${formatIsoDate(e.date, locale, { dateStyle: 'medium' })}</td><td>${e.flare ?? ''}</td><td>${e.mood ?? ''}</td><td>${e.sleep ?? ''}</td><td>${e.fatigue ?? ''}</td><td>${(e.notes ?? '').slice(0, 120)}</td></tr>`
    )
    .join('');
