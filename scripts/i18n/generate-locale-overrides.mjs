...
import { PLAN08_TIER_A_OVERRIDES } from '../lib/plan08-tier-a-overrides.mjs';
import { PLAN09_TIER_A_OVERRIDES } from '../lib/plan09-tier-a-overrides.mjs';
import { PLAN10_TIER_A_OVERRIDES } from '../lib/plan10-tier-a-overrides.mjs';
import { PLAN12_TIER_A_OVERRIDES } from '../lib/plan12-tier-a-overrides.mjs';
import { PLAN13_TIER_A_OVERRIDES } from '../lib/plan13-tier-a-overrides.mjs';
import { PLAN14_TIER_A_OVERRIDES } from '../lib/plan14-tier-a-overrides.mjs';
import { ACHIEVEMENTS_TIER_A_OVERRIDES } from '../lib/achievements-tier-a-overrides.mjs';
import { ENGAGEMENT_TIER_A_OVERRIDES } from '../lib/engagement-tier-a-overrides.mjs';
import { AI_ANALYSIS_TIER_A_OVERRIDES } from '../lib/ai-analysis-tier-a-overrides.mjs';
import { PLAN19_TIER_A_OVERRIDES } from '../lib/plan19-tier-a-overrides.mjs';
...
function applyTierATranslations(strings, locale) {
  ...
  const plan08 = PLAN08_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan08)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan09 = PLAN09_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan09)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan10 = PLAN10_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan10)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan12 = PLAN12_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan12)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan13 = PLAN13_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan13)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan14 = PLAN14_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan14)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const achievements = ACHIEVEMENTS_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(achievements)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const engagement = ENGAGEMENT_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(engagement)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const aiAnalysis = AI_ANALYSIS_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(aiAnalysis)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const plan19 = PLAN19_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(plan19)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const v2 = V2_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(v2)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  const v205 = V205_BP_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(v205)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  ...
  const v213 = V213_LOG_WIZARD_TIER_A_OVERRIDES[locale] || {};
  for (const [key, val] of Object.entries(v213)) {
    if (typeof val === 'string' && val.trim()) out[key] = val;
  }
  return out;
}
