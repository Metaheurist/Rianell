...
if (!fs.existsSync(embeddedPath)) {
  fail(`missing packages/shared/${I18N_PACKS_DIR}/policy-packs/v1.json — run node scripts/sync-policy-pack.mjs`);
} else {
...
if (!fs.existsSync(embeddedSrcPath)) {
  fail('missing policyPackData.mjs — run node scripts/sync-policy-pack.mjs');
} else {
  const src = fs.readFileSync(embeddedSrcPath, 'utf8');
  if (!src.includes('Generated by scripts/sync-policy-pack.mjs')) {
    fail('policyPackData.mjs must be generated by scripts/sync-policy-pack.mjs');
  }
