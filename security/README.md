...
- **`securityheaders-runs/run-*.md`** — One file per workflow run for history. **Retention:** keep `securityheaders-rianell.com.md` plus the **10 most recent** `run-*.md` files; older runs are pruned by `scripts/audit/fetch-securityheaders-report.mjs --prune-only` after each CI report (see `.github/workflows/ci.yml`).

Front matter may include **`securityheaders_scan_relay`** (`direct` | `allorigins` | `corsproxy.io` | `codetabs`) when relay HTML was used.

Generated by **`scripts/audit/fetch-securityheaders-report.mjs`** (`.github/workflows/ci.yml` → job **Security Headers report**). Commits use `[skip ci]` to avoid workflow loops.
