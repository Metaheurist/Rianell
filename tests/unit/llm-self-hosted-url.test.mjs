...
test('modelIdFromTier returns full onnx-community ids', () => {
  ...
  assert.equal(modelIdFromTier('tier3'), 'onnx-community/Qwen2.5-1.5B-Instruct');
});
...
test('buildSelfHostedModelFileUrl matches PWA /models/ layout', () => {
  ...
  assert.equal(
    url,
    'https://example.github.io/Health-app/models/onnx-community/Qwen2.5-0.5B-Instruct/resolve/main/config.json'
  );
});
...
test('buildSupabaseModelsPublicBase matches Storage public URL layout', async () => {
  ...
  assert.equal(
    file,
    'https://abc.supabase.co/storage/v1/object/public/llm-models/models/onnx-community/Qwen2.5-0.5B-Instruct/resolve/main/config.json'
  );
});
