...
To add latency checks later: run the server on a fixed port and measure `GET /` and `GET /api/supabase-status` with a small client (e.g. Node `http.request` or Python `urllib`) over multiple iterations, then write results here or extend `benchmark-runner/scripts/`.
