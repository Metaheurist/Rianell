...
- **Commit artifact fallback:** if `commit-app-build` push is rejected by repository file-size/quota limits (GH001/LFS-style rejection), workflow must fall back to a README-only metadata commit so release/post steps can proceed with updated build info.
