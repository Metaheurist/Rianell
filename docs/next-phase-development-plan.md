...
- [~] **Notifications** — RN Settings now has reminder enable/time/sound + snooze-interval prefs, permission status/request baseline, runtime capability visibility (including dismiss semantics availability), explicit action-policy/fallback copy for `log-now` / `later` / `default` / `unknown`, root action handling now suppresses immediate duplicate reminder-action bursts to avoid repeated routing, notification action parsing now normalizes cross-runtime identifier variants (case/separator differences) before mapping action semantics, snoozed reminder notification taps now map into the same open-app action flow (`default`), dismissed/close-style notification actions are safely ignored (`none`) so they do not trigger unintended fallback routing, and Settings now tracks/shows unknown reminder-action observations for the current app session (runtime-cause hint + total count + startup/live breakdown + startup/live percentage split + drift-status severity + dominant-source confidence hint + minimum-sample confidence warning + observability quality score + recommended next-check guidance + source trajectory context + trajectory stability note + last-seen time + latest source + reset control) to make long-tail runtime drift visible and manageable. **Open:** remaining OS-level long-tail semantics parity vs Capacitor/web paths (§4.2). **Docs checkpoint:** synced in `v1.45.99`.
