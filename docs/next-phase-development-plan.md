...
- CI can generate **RN CLI** native artifacts (Android debug APK + iOS Xcode project zip) and attach them to GitHub Releases. **`rncli-android-apk`** / **`rncli-ios-zip`** stamp **`version`** in `App build/RNCLI-Android/latest.json` and **`App build/iOS/latest.json`** (and iOS zip basenames **`Health-Tracker-ios-alpha-build-<N>`**) using **`github.run_number`**, aligned with **Server** EXE metadata and the **Web / PWA** README row for the same workflow run. (A prior sequential counter stayed at **1** when large-binary git pushes failed and only README was committed.)
...
- **Android (RN CLI)**:
  ...
  - `App build/RNCLI-Android/latest.json` (points to file + **`version`** = workflow **`run_number`**)
- **iOS (RN CLI emulator zip)**:
  - `App build/iOS/Health-Tracker-ios-alpha-build-<N>.zip` (**`<N>`** = **`github.run_number`**, same as Android `latest.json` **`version`**)
...
**Legacy (Capacitor, not rebuilt):** frozen metadata under **`App build/Android/`** and **`App build/Legacy/Capacitor-iOS/`** for README / release “Legacy” bundles only; release files are published with explicit `legacy-capacitor-*` names.
...
- [~] **Notifications** — RN Settings now has reminder enable/time/sound + snooze-interval prefs, permission status/request baseline, runtime capability visibility (including dismiss semantics availability), explicit action-policy/fallback copy for `log-now` / `later` / `default` / `unknown`, root action handling now suppresses immediate duplicate reminder-action bursts to avoid repeated routing, notification action parsing now normalizes cross-runtime identifier variants (case/separator differences) before mapping action semantics, snoozed reminder notification taps now map into the same open-app action flow (`default`), dismissed/close-style notification actions are safely ignored (`none`) so they do not trigger unintended fallback routing, and Settings now tracks/shows unknown reminder-action observations for the current app session (runtime-cause hint + total count + compact session summary + startup/live breakdown + startup/live percentage split + drift-status severity + dominant-source confidence hint + minimum-sample confidence warning + observability quality score + recommended next-check guidance + source trajectory context + trajectory stability note + last-seen time + latest source + reset control) to make long-tail runtime drift visible and manageable. **Open:** remaining OS-level long-tail semantics parity vs Capacitor/web paths (§4.2). **Docs checkpoint:** synced in `v1.46.1`.
...
- **`rncli-android-apk`** / **`rncli-ios-zip`** stamp RN **`latest.json`** / zip names with **`github.run_number`** (see `tests/unit/workflows-ci-rncli.test.mjs`).
...
- **Commit artifact fallback:** if `commit-app-build` push is rejected by repository file-size/quota limits (GH001/LFS-style rejection), workflow falls back to committing **README.md** plus **small `latest.json` files** under **`App build/RNCLI-Android/`**, **`App build/iOS/`**, and **`App build/Server/`** so RN/Server build numbers on Pages and in the README stay current even when large binaries cannot be pushed.
