...
- **Important:** The PWA already defines a full CSP in **`apps/pwa-webapp/index.html`**. If Cloudflare adds a **second, narrower** `Content-Security-Policy` header (for example only allowing `https://*.supabase.co` in `script-src`), the browser applies **both** policies and **blocks** scripts and styles that the meta tag allows (jsDelivr, Google Fonts, Font Awesome, ML libraries). Symptoms include console **script-src** / **style-src** violations and “Supabase library not loaded”. **Either remove** the HTTP CSP at the edge **or** match the meta policy exactly. Copy-paste guidance: **[security/cloudflare-headers-recommended.md](../security/cloudflare-headers-recommended.md)**.
