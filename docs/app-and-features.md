...
**Standing (2026-07-25):** Production **PWA** at [rianell.com](https://rianell.com/) · npm **v2.6.0** · no native store app · Smartlook **opt-in** · Visual pack polish tooling published but **not** applied to shipped sprites yet.
...
- **Purpose:** Optional EU session analytics (Smartlook) - **off by default**; only runs after explicit opt-in during first-run or in Settings → Privacy & region. Revoke anytime from Settings or the Consent dashboard. Blocked while local-only mode is on.
...
- **Summary note**: In-browser LLM (Transformers.js chat models: **Qwen2.5-1.5B-Instruct** tier 3-5, **Qwen2.5-0.5B-Instruct** tier 1-2 - multilingual, all offered UI locales) or rule-based fallback; context from analysis and logs; value highlighting in the UI.
