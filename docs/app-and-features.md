...
- **Dashboard title (MOTD)**: Main header shows a **message of the day** only (no user name). Preset lines are loaded from **`apps/pwa-webapp/motd.json`** at startup (short attributed quotations); **one line is chosen at random on each full page load** (stable for that session until the LLM may replace it). If the file is missing or offline, a minimal fallback is used. Curated quotes are the **primary** source (v1.47.1+): when AI is enabled and not deferred, the on-device LLM attempts a replacement on only **~30% of page loads** (one stable roll per session), with **cooler sampling** (temperature 0.7) and an **`isUsableMotdText`** relevance gate in `summary-llm.js` that rejects digits, tech/trivia vocabulary, and lines without motivational wording. Browser tab title stays **Rianell**. Edit **`apps/pwa-webapp/motd.json`** to change copy without editing **`app.js`**. On Home (dark theme), the title strip supports **tap / drag spin** (3D `rotateX`; rapid taps can complete a **full rotation** — see **`docs/styling.md`**).
