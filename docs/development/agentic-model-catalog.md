...
| `dual_12_16` | ~12 + ~16 GB | ≤ 19 GB |
| `dual_balanced` | Other multi-GPU | ≤ largest card |
...
- Live packs pass the effective profile into `resolvePackModel` so recommended models downshift to the largest **allowed** fit (e.g. 32B → 14B on `single_12`).
