...
- **No** app-level encryption for `localStorage` health logs.
...
3. **Cross-device** - same passphrase derives same key across browsers.
...
| Iterations / memory | OWASP 2023 profile; benchmark on low-end devices |
...
4. **Biometric unlock** - WebAuthn / platform authenticator wraps CEK?
...
| Cross-browser derive parity | Same ciphertext decrypt across browsers |
| Performance | KDF < 2s on mid-tier devices |
