# Benchmark comparison (history)

Generated at **2026-06-16T10:40:05.991Z**.

To change the default number of runs in the primary sections below, edit **[compare.config.json](./compare.config.json)** (`window`). GitHub does not support interactive dropdowns in Markdown; optional **collapsed sections** list alternate window sizes.

---

### Web / PWA

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:10 | b25a874 | ok | 13665 | 8784 | 71 | 27611604472 |
| 2026-06-16 06:03:27 | 52c24ba | ok | 12311 | 8257 | 67 | 27597550897 |
| 2026-06-16 05:54:51 | af742cc | ok | 13514 | 8334 | 59 | 27597185363 |
| 2026-06-15 23:06:25 | 8c88228 | ok | 14099 | 10944 | 158 | 27582159029 |
| 2026-06-15 22:51:54 | f1d81b5 | ok | 14091 | 10787 | 140 | 27581514757 |
| 2026-06-15 22:45:47 | f02224d | ok | 13178 | 9724 | 77 | 27581252223 |
| 2026-06-15 22:37:51 | cb092d9 | ok | 14089 | 10927 | 167 | 27580894614 |
| 2026-06-15 22:27:10 | 9c3f148 | ok | 13157 | 9703 | 69 | 27580413350 |
| 2026-06-15 22:12:56 | 910fc5a | ok | 14103 | 9955 | 148 | 27579705192 |
| 2026-06-15 21:57:35 | 1d3b3ff | ok | 13484 | 9879 | 90 | 27578918030 |

```mermaid
xychart-beta
    title "Web / PWA — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "ms" 0 --> 15514
    line [13665, 12311, 13514, 14099, 14091, 13178, 14089, 13157, 14103, 13484]
```
```mermaid
xychart-beta
    title "Web / PWA — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "ms" 0 --> 184
    line [71, 67, 59, 158, 140, 77, 167, 69, 148, 90]
```

### GitHub Pages

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:34 | b25a874 | ok | 13959 | 8781 | 73 | 27611604472 |
| 2026-06-16 06:03:49 | 52c24ba | ok | 12313 | 8259 | 57 | 27597550897 |
| 2026-06-16 05:55:14 | af742cc | ok | 13510 | 8332 | 55 | 27597185363 |
| 2026-06-15 23:06:48 | 8c88228 | ok | 14082 | 10499 | 125 | 27582159029 |
| 2026-06-15 22:52:17 | f1d81b5 | ok | 14090 | 10484 | 140 | 27581514757 |
| 2026-06-15 22:46:10 | f02224d | ok | 14084 | 10486 | 121 | 27581252223 |
| 2026-06-15 22:38:13 | cb092d9 | ok | 13204 | 9751 | 105 | 27580894614 |
| 2026-06-15 22:27:32 | 9c3f148 | ok | 13156 | 9704 | 90 | 27580413350 |
| 2026-06-15 22:13:18 | 910fc5a | ok | 14081 | 10479 | 133 | 27579705192 |
| 2026-06-15 21:57:58 | 1d3b3ff | ok | 13177 | 9724 | 98 | 27578918030 |

```mermaid
xychart-beta
    title "GitHub Pages — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "ms" 0 --> 15500
    line [13959, 12313, 13510, 14082, 14090, 14084, 13204, 13156, 14081, 13177]
```
```mermaid
xychart-beta
    title "GitHub Pages — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "ms" 0 --> 154
    line [73, 57, 55, 125, 140, 121, 105, 90, 133, 98]
```

### Capacitor (legacy)
_No history yet._

### Expo / RN bundles

Aggregates: **sum of gzip bytes** across all `.hbc` files per platform (stable for trends when chunk hashes change).

| date | sha | status | android_gzip | ios_gzip | run |
| --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:39:41 | b25a874 | ok | 3287424 | 3278199 | 27611604472 |
| 2026-06-16 06:04:44 | 52c24ba | ok | 3287423 | 3278191 | 27597550897 |
| 2026-06-16 05:55:48 | af742cc | ok | 3287424 | 3278197 | 27597185363 |
| 2026-06-15 23:07:16 | 8c88228 | ok | 3287869 | 3277689 | 27582159029 |
| 2026-06-15 22:52:59 | f1d81b5 | ok | 3287424 | 3278200 | 27581514757 |
| 2026-06-15 22:47:03 | f02224d | ok | 3287426 | 3278200 | 27581252223 |
| 2026-06-15 22:38:54 | cb092d9 | ok | 3287424 | 3278198 | 27580894614 |
| 2026-06-15 22:28:41 | 9c3f148 | ok | 3287424 | 3278199 | 27580413350 |
| 2026-06-15 22:13:34 | 910fc5a | ok | 3287869 | 3277688 | 27579705192 |
| 2026-06-15 21:58:45 | 1d3b3ff | ok | 3287424 | 3278198 | 27578918030 |

```mermaid
xychart-beta
    title "Expo — Android Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "bytes" 0 --> 3616656
    line [3287424, 3287423, 3287424, 3287869, 3287424, 3287426, 3287424, 3287424, 3287869, 3287424]
```
```mermaid
xychart-beta
    title "Expo — iOS Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff"]
    y-axis "bytes" 0 --> 3606021
    line [3278199, 3278191, 3278197, 3277689, 3278200, 3278200, 3278198, 3278199, 3277688, 3278198]
```

<details>
<summary>Last <strong>5</strong> runs (all platforms)</summary>

### Web / PWA

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:10 | b25a874 | ok | 13665 | 8784 | 71 | 27611604472 |
| 2026-06-16 06:03:27 | 52c24ba | ok | 12311 | 8257 | 67 | 27597550897 |
| 2026-06-16 05:54:51 | af742cc | ok | 13514 | 8334 | 59 | 27597185363 |
| 2026-06-15 23:06:25 | 8c88228 | ok | 14099 | 10944 | 158 | 27582159029 |
| 2026-06-15 22:51:54 | f1d81b5 | ok | 14091 | 10787 | 140 | 27581514757 |

```mermaid
xychart-beta
    title "Web / PWA — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "ms" 0 --> 15509
    line [13665, 12311, 13514, 14099, 14091]
```
```mermaid
xychart-beta
    title "Web / PWA — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "ms" 0 --> 174
    line [71, 67, 59, 158, 140]
```

### GitHub Pages

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:34 | b25a874 | ok | 13959 | 8781 | 73 | 27611604472 |
| 2026-06-16 06:03:49 | 52c24ba | ok | 12313 | 8259 | 57 | 27597550897 |
| 2026-06-16 05:55:14 | af742cc | ok | 13510 | 8332 | 55 | 27597185363 |
| 2026-06-15 23:06:48 | 8c88228 | ok | 14082 | 10499 | 125 | 27582159029 |
| 2026-06-15 22:52:17 | f1d81b5 | ok | 14090 | 10484 | 140 | 27581514757 |

```mermaid
xychart-beta
    title "GitHub Pages — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "ms" 0 --> 15500
    line [13959, 12313, 13510, 14082, 14090]
```
```mermaid
xychart-beta
    title "GitHub Pages — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "ms" 0 --> 154
    line [73, 57, 55, 125, 140]
```

### Capacitor (legacy)
_No history yet._

### Expo / RN bundles

Aggregates: **sum of gzip bytes** across all `.hbc` files per platform (stable for trends when chunk hashes change).

| date | sha | status | android_gzip | ios_gzip | run |
| --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:39:41 | b25a874 | ok | 3287424 | 3278199 | 27611604472 |
| 2026-06-16 06:04:44 | 52c24ba | ok | 3287423 | 3278191 | 27597550897 |
| 2026-06-16 05:55:48 | af742cc | ok | 3287424 | 3278197 | 27597185363 |
| 2026-06-15 23:07:16 | 8c88228 | ok | 3287869 | 3277689 | 27582159029 |
| 2026-06-15 22:52:59 | f1d81b5 | ok | 3287424 | 3278200 | 27581514757 |

```mermaid
xychart-beta
    title "Expo — Android Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "bytes" 0 --> 3616656
    line [3287424, 3287423, 3287424, 3287869, 3287424]
```
```mermaid
xychart-beta
    title "Expo — iOS Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5"]
    y-axis "bytes" 0 --> 3606021
    line [3278199, 3278191, 3278197, 3277689, 3278200]
```

</details>

<details>
<summary>Last <strong>20</strong> runs (all platforms)</summary>

### Web / PWA

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:10 | b25a874 | ok | 13665 | 8784 | 71 | 27611604472 |
| 2026-06-16 06:03:27 | 52c24ba | ok | 12311 | 8257 | 67 | 27597550897 |
| 2026-06-16 05:54:51 | af742cc | ok | 13514 | 8334 | 59 | 27597185363 |
| 2026-06-15 23:06:25 | 8c88228 | ok | 14099 | 10944 | 158 | 27582159029 |
| 2026-06-15 22:51:54 | f1d81b5 | ok | 14091 | 10787 | 140 | 27581514757 |
| 2026-06-15 22:45:47 | f02224d | ok | 13178 | 9724 | 77 | 27581252223 |
| 2026-06-15 22:37:51 | cb092d9 | ok | 14089 | 10927 | 167 | 27580894614 |
| 2026-06-15 22:27:10 | 9c3f148 | ok | 13157 | 9703 | 69 | 27580413350 |
| 2026-06-15 22:12:56 | 910fc5a | ok | 14103 | 9955 | 148 | 27579705192 |
| 2026-06-15 21:57:35 | 1d3b3ff | ok | 13484 | 9879 | 90 | 27578918030 |
| 2026-06-15 20:06:07 | 97c29c1 | ok | 14084 | 10132 | 135 | 27572035531 |
| 2026-06-15 19:42:51 | 4c6144b | ok | 14094 | 10816 | 167 | 27571371864 |
| 2026-06-15 19:30:21 | f167b31 | ok | 13501 | 9896 | 121 | 27570907674 |
| 2026-06-14 08:25:21 | afb25d4 | ok | 13531 | 9214 | 97 | 27493118482 |
| 2026-06-14 08:07:54 | 36e0b4b | ok | 13563 | 8738 | 140 | 27492730321 |
| 2026-06-14 01:01:58 | 5095a3b | ok | 13394 | 8646 | 158 | 27484132166 |
| 2026-06-13 16:55:37 | 84f6619 | ok | 12124 | 8314 | 20 | 27473022439 |
| 2026-06-13 11:09:28 | 68b4b12 | ok | 12007 | 8334 | 54 | 27465001801 |
| 2026-06-13 10:46:59 | d6308aa | ok | 12072 | 8348 | 110 | 27464520051 |
| 2026-06-12 19:00:16 | 75b5f2d | ok | 11911 | 7881 | 55 | 27436558960 |

```mermaid
xychart-beta
    title "Web / PWA — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa", "75b5f2d"]
    y-axis "ms" 0 --> 15514
    line [13665, 12311, 13514, 14099, 14091, 13178, 14089, 13157, 14103, 13484, 14084, 14094, 13501, 13531, 13563, 13394, 12124, 12007, 12072, 11911]
```
```mermaid
xychart-beta
    title "Web / PWA — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa", "75b5f2d"]
    y-axis "ms" 0 --> 184
    line [71, 67, 59, 158, 140, 77, 167, 69, 148, 90, 135, 167, 121, 97, 140, 158, 20, 54, 110, 55]
```

### GitHub Pages

| date | sha | status | LCP_ms | FCP_ms | TBT_ms | run |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:38:34 | b25a874 | ok | 13959 | 8781 | 73 | 27611604472 |
| 2026-06-16 06:03:49 | 52c24ba | ok | 12313 | 8259 | 57 | 27597550897 |
| 2026-06-16 05:55:14 | af742cc | ok | 13510 | 8332 | 55 | 27597185363 |
| 2026-06-15 23:06:48 | 8c88228 | ok | 14082 | 10499 | 125 | 27582159029 |
| 2026-06-15 22:52:17 | f1d81b5 | ok | 14090 | 10484 | 140 | 27581514757 |
| 2026-06-15 22:46:10 | f02224d | ok | 14084 | 10486 | 121 | 27581252223 |
| 2026-06-15 22:38:13 | cb092d9 | ok | 13204 | 9751 | 105 | 27580894614 |
| 2026-06-15 22:27:32 | 9c3f148 | ok | 13156 | 9704 | 90 | 27580413350 |
| 2026-06-15 22:13:18 | 910fc5a | ok | 14081 | 10479 | 133 | 27579705192 |
| 2026-06-15 21:57:58 | 1d3b3ff | ok | 13177 | 9724 | 98 | 27578918030 |
| 2026-06-15 20:06:30 | 97c29c1 | ok | 14085 | 10479 | 117 | 27572035531 |
| 2026-06-15 19:43:14 | 4c6144b | ok | 14088 | 10492 | 143 | 27571371864 |
| 2026-06-15 19:30:44 | f167b31 | ok | 13032 | 9598 | 92 | 27570907674 |
| 2026-06-14 08:25:47 | afb25d4 | ok | 13521 | 8708 | 65 | 27493118482 |
| 2026-06-14 08:08:20 | 36e0b4b | ok | 13383 | 8717 | 94 | 27492730321 |
| 2026-06-14 01:02:24 | 5095a3b | ok | 13388 | 8713 | 87 | 27484132166 |
| 2026-06-13 16:56:01 | 84f6619 | ok | 12127 | 8168 | 39 | 27473022439 |
| 2026-06-13 11:09:53 | 68b4b12 | ok | 12028 | 8175 | 77 | 27465001801 |
| 2026-06-13 10:47:24 | d6308aa | ok | 11904 | 8339 | 53 | 27464520051 |
| 2026-06-12 19:00:41 | 75b5f2d | ok | 11863 | 7882 | 46 | 27436558960 |

```mermaid
xychart-beta
    title "GitHub Pages — LCP (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa", "75b5f2d"]
    y-axis "ms" 0 --> 15500
    line [13959, 12313, 13510, 14082, 14090, 14084, 13204, 13156, 14081, 13177, 14085, 14088, 13032, 13521, 13383, 13388, 12127, 12028, 11904, 11863]
```
```mermaid
xychart-beta
    title "GitHub Pages — TBT (ms)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa", "75b5f2d"]
    y-axis "ms" 0 --> 158
    line [73, 57, 55, 125, 140, 121, 105, 90, 133, 98, 117, 143, 92, 65, 94, 87, 39, 77, 53, 46]
```

### Capacitor (legacy)
_No history yet._

### Expo / RN bundles

Aggregates: **sum of gzip bytes** across all `.hbc` files per platform (stable for trends when chunk hashes change).

| date | sha | status | android_gzip | ios_gzip | run |
| --- | --- | --- | --- | --- | --- |
| 2026-06-16 10:39:41 | b25a874 | ok | 3287424 | 3278199 | 27611604472 |
| 2026-06-16 06:04:44 | 52c24ba | ok | 3287423 | 3278191 | 27597550897 |
| 2026-06-16 05:55:48 | af742cc | ok | 3287424 | 3278197 | 27597185363 |
| 2026-06-15 23:07:16 | 8c88228 | ok | 3287869 | 3277689 | 27582159029 |
| 2026-06-15 22:52:59 | f1d81b5 | ok | 3287424 | 3278200 | 27581514757 |
| 2026-06-15 22:47:03 | f02224d | ok | 3287426 | 3278200 | 27581252223 |
| 2026-06-15 22:38:54 | cb092d9 | ok | 3287424 | 3278198 | 27580894614 |
| 2026-06-15 22:28:41 | 9c3f148 | ok | 3287424 | 3278199 | 27580413350 |
| 2026-06-15 22:13:34 | 910fc5a | ok | 3287869 | 3277688 | 27579705192 |
| 2026-06-15 21:58:45 | 1d3b3ff | ok | 3287424 | 3278198 | 27578918030 |
| 2026-06-15 20:07:14 | 97c29c1 | ok | 3266092 | 3258272 | 27572035531 |
| 2026-06-15 19:50:15 | 00011b8 | ok | 3266094 | 3258276 | 27571805119 |
| 2026-06-15 19:43:38 | 4c6144b | ok | 3266092 | 3258273 | 27571371864 |
| 2026-06-15 19:32:03 | f167b31 | ok | 3266093 | 3258270 | 27570907674 |
| 2026-06-14 08:26:02 | afb25d4 | ok | 3239886 | 3230867 | 27493118482 |
| 2026-06-14 08:08:52 | 36e0b4b | ok | 3240548 | 3230712 | 27492730321 |
| 2026-06-14 01:03:17 | 5095a3b | ok | 2511667 | 2504401 | 27484132166 |
| 2026-06-13 16:56:41 | 84f6619 | ok | 2411913 | 2405024 | 27473022439 |
| 2026-06-13 11:09:53 | 68b4b12 | ok | 2377489 | 2370491 | 27465001801 |
| 2026-06-13 10:47:46 | d6308aa | ok | 2377488 | 2370493 | 27464520051 |

```mermaid
xychart-beta
    title "Expo — Android Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "00011b8", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa"]
    y-axis "bytes" 0 --> 3616656
    line [3287424, 3287423, 3287424, 3287869, 3287424, 3287426, 3287424, 3287424, 3287869, 3287424, 3266092, 3266094, 3266092, 3266093, 3239886, 3240548, 2511667, 2411913, 2377489, 2377488]
```
```mermaid
xychart-beta
    title "Expo — iOS Hermes gzip total (bytes)"
    x-axis ["b25a874", "52c24ba", "af742cc", "8c88228", "f1d81b5", "f02224d", "cb092d9", "9c3f148", "910fc5a", "1d3b3ff", "97c29c1", "00011b8", "4c6144b", "f167b31", "afb25d4", "36e0b4b", "5095a3b", "84f6619", "68b4b12", "d6308aa"]
    y-axis "bytes" 0 --> 3606021
    line [3278199, 3278191, 3278197, 3277689, 3278200, 3278200, 3278198, 3278199, 3277688, 3278198, 3258272, 3258276, 3258273, 3258270, 3230867, 3230712, 2504401, 2405024, 2370491, 2370493]
```

</details>

